...
def get_file_category(file_path):
        ...
        try:
            ...
            for category, extensions in FILE_EXTENSIONS.items():
                if file_ext in extensions:
                    print(f"File {os.path.basename(file_path)} classificato per estensione '{file_ext}' come {category} (MIME: {mime_type})")
                    return category
                    
            print(f"File {os.path.basename(file_path)} non riconosciuto. MIME: {mime_type}, Estensione: {file_ext}")
                
        except Exception as e:
            print(f"Errore nel determinare il tipo MIME per {os.path.basename(file_path)}: {e}")
    ...
    for category, extensions in FILE_EXTENSIONS.items():
        if file_ext in extensions:
            print(f"File {os.path.basename(file_path)} classificato per estensione '{file_ext}' come {category}")
            return category
    ...
    def processing_task(self, zip_file_path, output_dir):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Phase 2: Preparazione per l'analisi
                self.update_status("Preparazione per l'analisi dei file...")
                ...
                # Build a list of all files for analysis
                file_list = []
                ...
                self.log(f"Trovati {total_files} file da analizzare")
                
                # Analisi in 3 fasi separate per garantire analisi completa
                # Fase 1: Ricerca file video
                self.update_status("Fase 1: Ricerca file video...")
                self.log("Fase 1: Iniziata ricerca file video")
                ...
                for i, file_path in enumerate(file_list):
                    ...
                    try:
                        ...
                        self.log(f"Errore nell'analisi del file {os.path.basename(file_path)}: {str(e)}")
                    
                    # Update progress (30-40%)
                    progress_percent = 30 + (i + 1) / total_files * 10
                    self.progress_var.set(progress_percent)
                    
                    if i % 10 == 0 or i == total_files - 1:
                        self.update_status(f"Ricerca video: {i+1}/{total_files}...")
                
                # Fase 2: Ricerca file audio
                self.update_status("Fase 2: Ricerca file audio...")
                self.log(f"Fase 2: Iniziata ricerca file audio (trovati {len(categorized_files['video'])} video)")
                ...
                for i, file_path in enumerate(file_list):
                    ...
                    progress_percent = 40 + (i + 1) / total_files * 10
                    self.progress_var.set(progress_percent)
                    
                    if i % 10 == 0 or i == total_files - 1:
                        self.update_status(f"Ricerca audio: {i+1}/{total_files}...")
                
                # Fase 3: Ricerca file immagine
                self.update_status("Fase 3: Ricerca immagini...")
                self.log(f"Fase 3: Iniziata ricerca immagini (trovati {len(categorized_files['audio'])} audio)")
                ...
                for i, file_path in enumerate(file_list):
                    ...
                    progress_percent = 50 + (i + 1) / total_files * 10
                    self.progress_var.set(progress_percent)
                    
                    if i % 10 == 0 or i == total_files - 1:
                        self.update_status(f"Ricerca immagini: {i+1}/{total_files}...")
                ...
                results = {}
                # Calcola il numero di categorie con file
                        ...
                        try:
                            with zipfile.ZipFile(output_zip_path, 'w') as zipf:
                                for i, (file_path, file_name) in enumerate(files):
                                    ...
                                    cat_progress_total = 60 + (processed_cats + cat_progress) / max(1, category_count) * 40
            ...
            if not self.cancelled:
                ...
                # Mostra tutte le categorie, incluso 'others'
